...
function decodeEntities(input: string): string {
  ...
  output = output.replace(/&#x([0-9a-fA-F]+);/g, (full: string, value: string) => {
    const codePoint = Number.parseInt(value, 16)
    return Number.isFinite(codePoint) ? String.fromCodePoint(codePoint) : full
  })
  output = output.replace(/&#(\d+);/g, (full: string, value: string) => {
    const codePoint = Number.parseInt(value, 10)
    return Number.isFinite(codePoint) ? String.fromCodePoint(codePoint) : full
  })
  return output
}
function normalizeLegacyComponentNames(input: string): string {
...
function normalizeLineBreakTokens(input: string): string {
  return input.replace(/\\+\s*:br\s*/gi, '<br />\n').replace(/\s*:br\s*/gi, '<br />\n')
}
function normalizeBoldLabelSpacing(input: string): string {
  return input.replace(/(\*\*[^*]+:\*\*)([a-zäöü])/giu, '$1 $2')
}
function normalizeDoubleAngleAutolinks(input: string): string {
  return input.replace(/<<([^>\n]+)>>/g, '<$1>')
}
function normalizeEscapedHardBreaks(input: string): string {
  return input.replace(/[ \t]*\\[ \t]*\r?\n[ \t]*\r?\n/g, ' <br />\n').replace(/[ \t]*\\[ \t]*(?=\r?\n)/g, ' <br />')
}
export function cmsMdcNormalize(input: unknown): string {
