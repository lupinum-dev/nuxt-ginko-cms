 ...
 *
 * Routes:
 * - GET /api/cms/:collection - List items
 * - GET /api/cms/:collection/:slug - Get single item
 */
...
export default defineEventHandler(async (event) => {
  ...
  // Extract path from the request URL (everything after /api/cms/)
  const requestUrl = getRequestURL(event)
  const fullPath = requestUrl.pathname.replace(/^\/api\/cms\/?/, '')
