 ...
 * ```
 */
export function useCmsItem<T extends CmsItem = CmsItem>(
  collectionSlug: string,
  itemSlug: string,
  options: UseCmsItemOptions = {},
): AsyncData<(T & { _fallback: Record<string, boolean> }) | null, Error | null> {
  ...
  return useAsyncData<(T & { _fallback: Record<string, boolean> }) | null>(
    cacheKey,
    async () => {
      const locale = getLocale()
      // Preview mode: fetch from API
      if (cmsConfig.preview) {
        return fetchFromApi(collectionSlug, itemSlug, locale, options, cmsConfig) as Promise<(T & { _fallback: Record<string, boolean> }) | null>
      }

      // Production mode: read from cache
      return fetchFromCache(collectionSlug, itemSlug, locale, cmsConfig) as Promise<(T & { _fallback: Record<string, boolean> }) | null>
    },
    {
      watch: [currentLocale],
    },
 ...
 * Uses the /api/cms proxy to keep API key secure
 */
async function fetchFromApi(
  ...
  const url = `/api/cms/${collectionSlug}/${itemSlug}?${params.toString()}`
